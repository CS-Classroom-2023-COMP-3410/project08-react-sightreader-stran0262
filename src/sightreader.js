...
// Compare the note expected to be played against the note currently detected on the mic
function check_note() {
...
function load_abc_file(filename) {
    ...
    $.ajax({
        ...
        success: function (data) {
            original_loaded_abc = preprocess_abc_data(data);
            loaded_abc_filename = filename;
            loaded_filename_display.textContent = filename;
            $('#abc-textarea').val(original_loaded_abc);
...
function preprocess_abc_data(data) {
    ...
    let headers = [];
    let notes = [];
    let ignoredHeaders = new Set(['T', 'C', 'Z', 'S', 'N', 'G', 'O', 'H', 'I', 'P', 'W', 'F', 'B']);
